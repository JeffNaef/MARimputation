...
for (s in 1:10){
  ...
  YminusY <- rep(0, length(methods))
  names(escore)<-methods
  names(ediff)<-methods
  names(YminusY) <- methods
  for (method in methods){
    ...
    for (j in 1:m){
      ...
      YminusY[method] <- scoringRules:::esC_xx(t(X), w=rep(1/nrow(X),nrow(X)))
      
    }
    escore[method] <- 1/m*escore[method]
    ediff[method] <- 1/m*ediff[method]
    YminusY[method] <- 1/m*YminusY[method]
  }
  ...
  Results[[s]] <- list(energy.score=escore, ediff.score=ediff, YminusY=YminusY)
...
## Analysis
##2*E[e(H,P^*)]=E[|| X-X' ||]-2*E[|| X -Y || ] 
energydata<-2*t(sapply(1:length(Results), function(j) Results[[j]]$energy.score))
meanvalsenergy<- colMeans(energydata)

## - d(H,P^*)=E[|| X-X' ||]-2*E[|| X -Y || ] + E[|| Y -Y' ||]
diffdata<- -t(sapply(1:length(Results), function(j) Results[[j]]$ediff.score))
meanvalsdiff<- colMeans(energydata)

## E[|| Y -Y' ||] separately
Ydata<- t(sapply(1:length(Results), function(j) Results[[j]][[3]]))
meanvalsdiff<- colMeans(Ydata)


par(mfrow=c(3,1))
boxplot(energydata[,order(energydata)])
boxplot(Ydata[,order(meanvalsenergy)])
boxplot(diffdata[,order(diffdata)])
...
filename ="energyscorevsenergydistance"
